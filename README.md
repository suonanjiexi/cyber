...
* **分层中间件**：支持全局、路由组和路由特定的中间件，实现不同粒度的控制
* **丰富内置中间件**：提供JWT认证、限流、缓存、指标收集等实用中间件
* **上下文管理**：请求上下文贯穿整个处理流程
* **数据验证**：内置请求数据验证功能
* **错误处理**：统一的错误处理机制
* **超时重试**：支持请求超时和自动重试
...
1. 首先执行最先添加的中间件的前置逻辑
2. 然后执行下一个中间件的前置逻辑，以此类推
3. 执行最终的处理函数
4. 执行最后添加的中间件的后置逻辑
5. 执行倒数第二个中间件的后置逻辑，以此类推
...
5. **RateLimiter**：使用令牌桶算法限制请求频率
6. **JWTAuth**：JSON Web Token身份认证
7. **ResponseCache**：缓存API响应
8. **Metrics**：收集HTTP请求指标
...
token, err := middleware.GenerateToken(
    ...
    map[string]interface{}{"permissions": []string{"read", "write"}}, 
    config
)
```
...
config := middleware.RateLimiterConfig{
    Rate: 5.0,     // 每秒5个请求
    Capacity: 10.0, // 最多积累10个令牌
    Timeout: 500 * time.Millisecond, // 超出限制时等待500ms
}
api.Use(func(next cyber.HandlerFunc) cyber.HandlerFunc {
    return middleware.RateLimiterWithConfig(config, next)
})
```
...
config := middleware.CacheConfig{
    TTL: 10 * time.Minute,
    CacheStatusCodes: []int{200, 304},
}
app.GET("/api/products", listProducts, func(next cyber.HandlerFunc) cyber.HandlerFunc {
    return middleware.ResponseCacheWithConfig(config, next)
})
```
...
// 注册指标处理器
middleware.RegisterMetricsHandler(app)
middleware.RegisterMetricsViewHandler(app)
```
...
func main() {
	...
	app.Use(middleware.Logger)
	app.Use(middleware.Cors)
	app.Use(middleware.RateLimiter) // 限速中间件
	app.Use(middleware.MetricsMiddleware) // 指标中间件
	...
	{
		// API组中间件
		api.Use(middleware.JWTAuth) // JWT认证
		
		users := api.Group("/users")
		{
			// 用户API路由
			users.GET("", getUsers)
			users.POSTWithMiddleware("", createUser, middleware.Timeout) // 添加超时中间件
			users.GETWithMiddleware("/:id", getUserByID, middleware.ResponseCache) // 添加响应缓存
		}
	}
...
6. **高效缓存**：响应缓存使用优化的内存存储，支持自动清理过期项
...
6. **选择合适的中间件级别**：根据需要选择全局、组或路由级别的中间件
7. **性能考虑**：高频使用的中间件应尽量轻量高效
